...
describe.skip('world_state_db', ()=>{
    let worldStateDb;
    beforeEach(async ()=>{
        (worldStateDb = new WorldStateDb(`/tmp/world_state_db_${randomBytes(32).toString('hex')}.db`)).destroy(), await worldStateDb.start();
    }), afterEach(()=>{
        worldStateDb.stop(), worldStateDb.destroy();
    }), it('should be initialized with correct metadata', ()=>{
        expect(worldStateDb.getRoot(0)).toEqual(WorldStateConstants.EMPTY_DATA_ROOT), expect(worldStateDb.getRoot(1)).toEqual(WorldStateConstants.EMPTY_NULL_ROOT), expect(worldStateDb.getRoot(2)).toEqual(WorldStateConstants.EMPTY_ROOT_ROOT), expect(worldStateDb.getRoot(3)).toEqual(WorldStateConstants.EMPTY_DEFI_ROOT), expect(worldStateDb.getSize(0)).toBe(BigInt(0)), expect(worldStateDb.getSize(1)).toBe(BigInt(0)), expect(worldStateDb.getSize(2)).toBe(BigInt(1)), expect(worldStateDb.getSize(3)).toBe(BigInt(0));
        ...
        let path = (await worldStateDb.getHashPath(0, BigInt(0))).data, expectedFirst = MerkleTree.ZERO_ELEMENT, expectedLast = '02a12922daa0fe8d05620d98096220a86d9ebf4d9552dc0fbd3862b9c48f7ab9';
