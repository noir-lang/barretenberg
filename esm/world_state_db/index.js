...
export class WorldStateDb {
    ...
    async getHashPath_(treeId, index) {
        ...
        this.proc.stdin.write(buffer);
        let depth = (await this.stdout.read(4)).readUInt32BE(0), result = await this.stdout.read(64 * depth), path = new HashPath();
    ...
    async batchPut_(entries) {
        let bufs = entries.map((e)=>Buffer.concat([
                ...
                toBufferBE(e.index, 32),
                e.value
            ])), buffer = Buffer.concat([
            Buffer.from([
                Command.BATCH_PUT
            ]),
            serializeBufferArrayToVector(bufs)
        ]);
    ...
    async launch() {
        await mkdirp('./data');
        let proc = this.proc = spawn(this.binPath, [
            this.dbPath
        ]);
        ...
        }), proc.on('error', console.log), this.stdout = new PromiseReadable(this.proc.stdout), await this.readMetadata();
    }
    async readMetadata() {
        ...
        this.sizes[0] = toBigIntBE(dataSize), this.sizes[1] = toBigIntBE(nullifierSize), this.sizes[2] = toBigIntBE(rootSize), this.sizes[3] = toBigIntBE(defiSize);
    }
    ...
    constructor(dbPath = './data/world_state.db'){
        this.dbPath = dbPath, this.stdioQueue = new MemoryFifo(), this.roots = [], this.sizes = [], this.binPath = '../barretenberg/build/bin/db_cli';
    }
}
