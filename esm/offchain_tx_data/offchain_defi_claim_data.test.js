...
describe('OffchainDefiClaimData', ()=>{
    it('convert offchain defi claim data to and from buffer', ()=>{
        let userData = new OffchainDefiClaimData(), buf = userData.toBuffer();
        expect(buf.length).toBe(OffchainDefiClaimData.SIZE), expect(OffchainDefiClaimData.fromBuffer(buf)).toEqual(userData);
