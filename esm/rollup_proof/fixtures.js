...
let randomCommitment = ()=>randomBytes(32), randomNullifier = ()=>randomBytes(32), randomInt = ()=>Buffer.concat([
        Buffer.alloc(28),
        randomBytes(4)
    ]);
...
export const randomInnerProofData = (proofId = ProofId.SEND)=>{
    switch(proofId){
        ...
        default:
            return new InnerProofData(proofId, randomCommitment(), randomCommitment(), randomNullifier(), [
                ProofId.ACCOUNT,
...
export const createRollupProofData = (innerProofs)=>{
    let bridgeIds = [
        ...
        ...Array(RollupProofData.NUM_BRIDGE_CALLS_PER_BLOCK)
    ].map(()=>randomBytes(32)), assetIds = [
        ...Array(RollupProofData.NUMBER_OF_ASSETS)
    ].map(()=>0), totalTxFees = [
        ...Array(RollupProofData.NUMBER_OF_ASSETS)
    ].map(()=>BigInt(0));
    return new RollupProofData(randomBytes(4).readUInt32BE(0), innerProofs.length, randomBytes(4).readUInt32BE(0), randomBytes(32), randomBytes(32), randomBytes(32), randomBytes(32), randomBytes(32), randomBytes(32), randomBytes(32), randomBytes(32), bridgeIds, defiDepositSums, assetIds, totalTxFees, defiInteractionNotes, randomBytes(32), randomBytes(32), innerProofs.length, innerProofs);
};
