"use strict";
var RollupProofDataFields, RollupProofDataOffsets;
...
const _crypto = require("crypto"), _clientProofs = require("../client_proofs"), _serialize = require("../serialize"), _decodeInnerProof = require("./decode_inner_proof"), _encodeInnerProof = require("./encode_inner_proof"), _innerProof = require("./inner_proof"), _ = require("."), _bigintBuffer = require("../bigint_buffer"), _bridgeId = require("../bridge_id");
...
}(RollupProofDataFields || (RollupProofDataFields = {})), function(RollupProofDataOffsets) {
...
const parseHeaderInputs = (proofData)=>{
    ...
    let defiDepositSums = [];
    for(let i1 = 0; i1 < RollupProofData.NUM_BRIDGE_CALLS_PER_BLOCK; ++i1)defiDepositSums.push((0, _bigintBuffer.toBigIntBE)(proofData.slice(startIndex, startIndex + 32))), startIndex += 32;
    let assetIds = [];
    for(let i2 = 0; i2 < RollupProofData.NUMBER_OF_ASSETS; ++i2)assetIds.push(proofData.readUInt32BE(startIndex + 28)), startIndex += 32;
    let totalTxFees = [];
    for(let i3 = 0; i3 < RollupProofData.NUMBER_OF_ASSETS; ++i3)totalTxFees.push((0, _bigintBuffer.toBigIntBE)(proofData.slice(startIndex, startIndex + 32))), startIndex += 32;
...
class RollupProofData {
    get rollupHash() {
        if (!this.rollupHash_) {
            ...
            this.rollupHash_ = (0, _crypto.createHash)('sha256').update(Buffer.concat(allTxIds)).digest();
        }
        return this.rollupHash_;
    }
    toBuffer() {
        return Buffer.concat([
            (0, _serialize.numToUInt32BE)(this.rollupId, 32),
            (0, _serialize.numToUInt32BE)(this.rollupSize, 32),
            (0, _serialize.numToUInt32BE)(this.dataStartIndex, 32),
            this.oldDataRoot,
            this.newDataRoot,
            this.oldNullRoot,
            this.newNullRoot,
            ...
            this.newDefiRoot,
            ...this.bridgeIds,
            ...this.defiDepositSums.map((v)=>(0, _bigintBuffer.toBufferBE)(v, 32)),
            ...this.assetIds.map((a)=>(0, _serialize.numToUInt32BE)(a, 32)),
            ...this.totalTxFees.map((a)=>(0, _bigintBuffer.toBufferBE)(a, 32)),
            ...this.defiInteractionNotes,
            this.prevDefiInteractionHash,
            this.rollupBeneficiary,
            (0, _serialize.numToUInt32BE)(this.numRollupTxs, 32),
            ...this.innerProofData.map((p)=>p.toBuffer())
        ]);
    }
    getTotalDeposited(assetId) {
        return this.innerProofData.filter((p)=>p.proofId === _clientProofs.ProofId.DEPOSIT).map((p)=>new _.RollupDepositProofData(p)).filter((p)=>p.assetId == assetId).reduce((a, p)=>a + p.publicValue, BigInt(0));
    }
    getTotalWithdrawn(assetId) {
        return this.innerProofData.filter((p)=>p.proofId === _clientProofs.ProofId.WITHDRAW).map((p)=>new _.RollupWithdrawProofData(p)).filter((p)=>p.assetId == assetId).reduce((a, p)=>a + p.publicValue, BigInt(0));
    }
    getTotalDefiDeposit(assetId) {
        return this.bridgeIds.map((bridgeId, i)=>_bridgeId.BridgeId.fromBuffer(bridgeId).inputAssetIdA === assetId || _bridgeId.BridgeId.fromBuffer(bridgeId).inputAssetIdB === assetId ? this.defiDepositSums[i] : BigInt(0)).reduce((acc, val)=>acc + val, BigInt(0));
    }
    ...
    encode() {
        ...
        this.innerProofData.forEach((p, i)=>{
            p.proofId !== _clientProofs.ProofId.PADDING && (lastNonEmptyIndex = i);
        });
        let numRealTxns = lastNonEmptyIndex + 1, encodedInnerProof = this.innerProofData.filter((p, i)=>i < numRealTxns).map((p)=>(0, _encodeInnerProof.encodeInnerProof)(p));
        return Buffer.concat([
            (0, _serialize.numToUInt32BE)(this.rollupId, 32),
            (0, _serialize.numToUInt32BE)(this.rollupSize, 32),
            (0, _serialize.numToUInt32BE)(this.dataStartIndex, 32),
            ...
            this.rollupBeneficiary,
            (0, _serialize.numToUInt32BE)(this.numRollupTxs, 32),
            (0, _serialize.numToUInt32BE)(numRealTxns),
            (0, _serialize.numToUInt32BE)(Buffer.concat(encodedInnerProof).length),
            ...encodedInnerProof
        ]);
    }
    ...
    static getTxIdsFromBuffer(proofData) {
        ...
        return Array.from({
            ...
            let innerProofStart = startIndex + i * _innerProof.InnerProofData.LENGTH;
            return (0, _clientProofs.createTxId)(proofData.slice(innerProofStart, innerProofStart + _innerProof.InnerProofData.LENGTH));
        }).filter((id)=>!id.equals(_innerProof.InnerProofData.PADDING.txId));
    }
    getNonPaddingProofs() {
    ...
    static fromBuffer(proofData) {
        ...
        for(let i = 0; i < rollupSize; ++i){
            let innerData = proofData.slice(startIndex, startIndex + _innerProof.InnerProofData.LENGTH);
            innerProofData[i] = _innerProof.InnerProofData.fromBuffer(innerData), startIndex += _innerProof.InnerProofData.LENGTH;
        }
    ...
    static randomData(rollupId, numTxs, dataStartIndex = 0, innerProofData, bridgeIds = []) {
        let ipd = void 0 === innerProofData ? Array(numTxs).fill(0).map(()=>_innerProof.InnerProofData.fromBuffer(Buffer.alloc(_innerProof.InnerProofData.LENGTH))) : innerProofData;
    ...
    static decode(encoded) {
        ...
        for(; innerProofDataLength > 0;){
            let innerProof = (0, _decodeInnerProof.decodeInnerProof)(encoded.slice(startIndex));
        ...
        for(let i = innerProofData.length; i < rollupSize; ++i)innerProofData.push(_innerProof.InnerProofData.PADDING);
        return new RollupProofData(rollupId, rollupSize, dataStartIndex, oldDataRoot, newDataRoot, oldNullRoot, newNullRoot, oldDataRootsRoot, newDataRootsRoot, oldDefiRoot, newDefiRoot, bridgeIds, defiDepositSums, assetIds, totalTxFees, defiInteractionNotes, prevDefiInteractionHash, rollupBeneficiary, numRollupTxs, innerProofData);
    }
