...
const _levelup = _interopRequireDefault(require("levelup")), _memdown = _interopRequireDefault(require("memdown")), _ = require("."), _pedersen = require("../crypto/pedersen"), _wasm = require("../wasm");
function _interopRequireDefault(obj) {
    return obj && obj.__esModule ? obj : {
        default: obj
    };
}
const expectSameTrees = async (tree1, tree2)=>{
...
describe('merkle_tree', ()=>{
    ...
    let values = [];
    beforeAll(async ()=>{
        await (barretenberg = new _wasm.BarretenbergWasm()).init(), pedersen = new _pedersen.SinglePedersen(barretenberg);
        for(let i = 0; i < 32; ++i){
            let v = Buffer.alloc(32, 0);
            v.writeUInt32BE(i, 28), values[i] = v;
        }
    }), it('should have correct root', async ()=>{
        let db = (0, _levelup.default)((0, _memdown.default)()), e00 = _.MerkleTree.ZERO_ELEMENT, e01 = values[1], e02 = values[2], e03 = values[3], e10 = pedersen.compress(e00, e01), e11 = pedersen.compress(e02, e03), root = pedersen.compress(e10, e11), tree = await _.MerkleTree.new(db, pedersen, 'test', 2);
        for(let i = 0; i < 4; ++i)await tree.updateElement(i, values[i]);
        let expected = new _.HashPath([
            [
                e00,
                e01
            ],
        ...
        expect(await tree.getHashPath(0)).toEqual(expected), expect(await tree.getHashPath(1)).toEqual(expected), expected = new _.HashPath([
            [
                e02,
                e03
            ],
        ...
        let db = (0, _levelup.default)((0, _memdown.default)()), tree = await _.MerkleTree.new(db, pedersen, 'test', 32), root = tree.getRoot();
        ...
        let db = (0, _levelup.default)((0, _memdown.default)()), tree = await _.MerkleTree.new(db, pedersen, 'test', 10);
        ...
        let db1 = (0, _levelup.default)((0, _memdown.default)()), tree1 = await _.MerkleTree.new(db1, pedersen, 'test', 10);
        for(let i1 = 0; i1 < values.length; ++i1)await tree1.updateElement(i1, values[i1]);
        let db2 = (0, _levelup.default)((0, _memdown.default)()), tree2 = await _.MerkleTree.new(db2, pedersen, 'test', 10);
        ...
        let values = Array(6).fill(Buffer.alloc(32, 0)), db1 = (0, _levelup.default)((0, _memdown.default)()), tree1 = await _.MerkleTree.new(db1, pedersen, 'test', 10);
        ...
        let levelDown = (0, _memdown.default)(), db = (0, _levelup.default)(levelDown), tree = await _.MerkleTree.new(db, pedersen, 'test', 10);
        for(let i = 0; i < 4; ++i)await tree.updateElement(i, values[i]);
        let db2 = (0, _levelup.default)(levelDown), tree2 = await _.MerkleTree.fromName(db2, pedersen, 'test');
        ...
        let db = (0, _levelup.default)((0, _memdown.default)());
        await expect((async ()=>{
            await _.MerkleTree.fromName(db, pedersen, 'a_whole_new_tree');
        })()).rejects.toThrow();
    }), it('should be able to sync the latest status from db', async ()=>{
        let levelDown = (0, _memdown.default)(), db1 = (0, _levelup.default)(levelDown), tree1 = await _.MerkleTree.new(db1, pedersen, 'test', 10), db2 = (0, _levelup.default)(levelDown), tree2 = await _.MerkleTree.new(db2, pedersen, 'test', 10);
        ...
        let hashPath = await tree.getHashPath(0), buf = hashPath.toBuffer(), recovered = _.HashPath.fromBuffer(buf);
        expect(recovered).toEqual(hashPath);
        let deserialized = _.HashPath.deserialize(buf);
        ...
        let dummyData = Buffer.alloc(23, 1), paddedBuf = Buffer.concat([
            dummyData,
            buf
        ]), recovered2 = _.HashPath.fromBuffer(paddedBuf, 23);
        expect(recovered2).toEqual(hashPath);
        let deserialized2 = _.HashPath.deserialize(buf);
